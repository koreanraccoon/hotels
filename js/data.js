...
function addItem(data){
    let element = [];
    let slicedData;
    slicedData = $filterdData.slice($added, $added += $addItemCounter);
    ...
    $.each(slicedData, function(index, item){
        ...
        let itemHTML = `
                <li>
                <div><a href="javascript:" class="galleryB">
                    <span class="g-video">
                       ${sw}
                    </span>
        ...
        if($added<$allData.length){
            ...
            $loadMoreBtn.css({backgroundColor:'#384244', color:'#dee4e3', border:'1px solid #5e686a'}).text('END');
        }
    })
